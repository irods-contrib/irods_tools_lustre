...
#  python irods_consortium_continuous_integration_build_hook.py --output_root_directory=<dir> --irods_packages_root_directory=<dir>
...
def main(output_root_directory, irods_packages_root_directory):
    ...
    lustre_irods_connector_build_directory = tempfile.mkdtemp(prefix='lustre_irods_connector_build_directory')
    print(lustre_irods_connector_build_directory)
    lustre_irods_connector_path = '%s/%s' % (os.path.dirname(os.path.realpath(__file__)), 'lustre_irods_connector')
    irods_lustre_plugin_build_directory = tempfile.mkdtemp(prefix='irods_lustre_plugin_build_directory')
    print(irods_lustre_plugin_build_directory)
    irods_lustre_plugin_path = '%s/%s' % (os.path.dirname(os.path.realpath(__file__)), 'irods_lustre_plugin')

    irods_python_ci_utilities.subprocess_get_output(['cmake', lustre_irods_connector_path], check_rc=True, cwd=lustre_irods_connector_build_directory)
    irods_python_ci_utilities.subprocess_get_output(['make', '-j', str(multiprocessing.cpu_count())], check_rc=True, cwd=lustre_irods_connector_build_directory)
    irods_python_ci_utilities.subprocess_get_output(['cmake', irods_lustre_plugin_path], check_rc=True, cwd=irods_lustre_plugin_build_directory)
    irods_python_ci_utilities.subprocess_get_output(['make', '-j', str(multiprocessing.cpu_count()), 'package'], check_rc=True, cwd=irods_lustre_plugin_build_directory)
    if output_root_directory:
        copy_output_packages(irods_lustre_plugin_build_directory, output_root_directory)

    print(str(['cp', '%s/lustre_irods_connector' % lustre_irods_connector_build_directory, '/bin']))
    irods_python_ci_utilities.subprocess_get_output(['cp', '%s/lustre_irods_connector' % lustre_irods_connector_build_directory, '/bin'])
